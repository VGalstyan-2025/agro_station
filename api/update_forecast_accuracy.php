...
if (
    ...
    // target_date-ի համար վերցնում ենք ԱՄԵՆԱՎԵՐՋԻՆ forecast-ը
    $sql = "SELECT id, forecast_eto, forecast_etc
            FROM forecast_history
            WHERE target_date = ?
            ORDER BY predicted_on DESC, id DESC
            LIMIT 1";
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if ($actual_eto > 0) {
            ...
            if ($base_eto > 0) {
                ...
                if ($accuracy_percent_eto < 0) $accuracy_percent_eto = 0;
            } else {
                $error_percent_eto = 0;
        ...
        if ($stmt2->execute()) {
            echo "Accuracy updated successfully for target_date = " . $target_date;
        } else {
            echo "Update failed";
        }
        ...
        echo "Forecast not found for target date";
    }
